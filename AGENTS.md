...
- `book_html.py` detects stale PDF/HTML pairs, repairs generated reflow HTML and audits all supported editions.
- `convert_book_pdf.py` converts one PDF, or the full collection, into reflowable HTML and extracted assets. It requires Poppler's `pdftohtml` command.
...
Whenever a cover is added or changed, regenerate all thumbnails and then rebuild the index:
...
Use the following workflow whenever a book PDF is added or changed:
...
2. Convert each reported PDF to its matching reflowable HTML file:
   ...
   ```bash
   python3 docs/content/tools/convert_book_pdf.py docs/content/RO/Book_RO.pdf
   ```
   ...
   To reconvert the entire collection in parallel after a converter change:
...
Every generated edition begins with an English warning explaining that figures or formatting may be missing or simplified and linking to the matching PDF. Keep this warning, the repair marker, the standalone reader CSS/JavaScript and the analytics script exactly once.
